...
/// <summary>
/// Incremental source generator that renames all identifiers in LibVLC.Native.* additional files
/// from snake_case / libvlc_* C-style naming to .NET PascalCase conventions.
/// </summary>
[Generator]
public sealed class SnakeCaseToPascalCaseGenerator : IIncrementalGenerator
{
    public static readonly DiagnosticDescriptor InfoDescriptor = new DiagnosticDescriptor(
        id: "LVSG001",
        title: "LibVLC SourceGenerator Info",
        messageFormat: "{0}",
        category: "LibVLC.SourceGenerator",
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var nativeFiles = context.AdditionalTextsProvider
            .Where(static file => Path.GetFileName(file.Path)
                .StartsWith("LibVLC.Interop.cs", StringComparison.OrdinalIgnoreCase));
    ...
    public const string RootNamespace = @"LibVLCSharp.AutoGen";
...
/// <summary>
/// Syntax walker that collects rename mappings needed to convert LibVLC identifiers
...
public sealed class RenameVisitor : CSharpSyntaxRewriter
{
    ...
    public override SyntaxNode? VisitIdentifierName(IdentifierNameSyntax node)
    {
        ...
        // Inline type references like libvlc_media_t that were not captured in GlobalRenames.
        if (name.StartsWith("libvlc_", StringComparison.OrdinalIgnoreCase) && name.EndsWith("_t", StringComparison.Ordinal))
...
internal static class NameConverter
{
    ...
    public static string ConvertTypeName(string name) =>
        name.RemovePrefix("libvlc_", StringComparison.OrdinalIgnoreCase).RemoveSuffix("_t").ToPascalCase().AddPrefix("Vlc");
    public static string ConvertMethodName(string name) =>
        name.RemovePrefix("libvlc_", StringComparison.OrdinalIgnoreCase).ToPascalCase().AddPrefix("Vlc");
    ...
    public static string ConvertConstantFieldName(string name)
    {
        ...
        if (name.All(ch => char.IsUpper(ch) || char.IsDigit(ch) || ch == '_'))
            return name;
        return name.RemovePrefix("libvlc_", StringComparison.OrdinalIgnoreCase).ToPascalCase();//.AddPrefix("Vlc");
    }

    public static string ConvertClassName(string name) =>
        string.Equals(name, "libvlc", StringComparison.OrdinalIgnoreCase) ? "LibVLC" : ConvertTypeName(name);
